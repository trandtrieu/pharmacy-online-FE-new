...
import { faHourglassEnd, faStopwatch20, faTicket, faTimesSquare } from "@fortawesome/free-solid-svg-icons";
import { faTimesCircle, faTimesRectangle } from "@fortawesome/free-regular-svg-icons";
...
const VoucherComponent = () => {
    ...
    return (
        <div className="container-fluid text-center">
            <img src={`${VOUCHER_IMGS_URL}/vouchervip.jpg`} className="rounded" height="550px" width="85%" alt="" />
                ...
                {discounts.map((d) => (
                    <div key={d.id} className="row col-md-6">
                        ...
                        <div className="col-md-7 mt-5">
                            ...
                            <button className="btn btn-danger" onClick={() => saveToAccount(d.id)}>
                                Save
                            </button> {" "}
                            <button className="btn btn-warning" onClick={() => modelOpen(d.id)}>
                                Details
                            </button>
                        </div>
                    </div>
            ...
            <ReactModal isOpen={isOpen} style={customStyles}>
                        ...
                        return (
                            <div key={discount.id} className="container-fluid">
                                <div className="bg-primary rounded p-1 pl-3 mb-3">
                                    <h4 className="text-success"><FontAwesomeIcon icon={faTicket} /> {discount.code}</h4>
                                    <p className="text-danger">{calculateRemainingTime(discount.expiryDate)} <FontAwesomeIcon icon={faHourglassEnd} /></p>
                                    <p>Sale off {discount.discountPercentage}% for {discount.condition}k minimun order</p>
                                </div>

                                <div>
                                    <h6>Applies to products</h6>
                                    <p>Products that are restricted from running promotions according to State regulations will not be displayed if they are in the selected product list.</p>
                                </div>
                                <div>
                                    <h6>Payment</h6>
                ...
                <button style={{ float: "right" }} onClick={() => modelOpen(null)} className="btn btn-primary ">
                    Close
                </button>
            </ReactModal>
        </div>
