...
class ProfileComponent extends Component {
  constructor(props) {
    ...
    this.deleteDelivery_Address = this.deleteDelivery_Address.bind(this)
    this.getProvinces = this.getProvinces.bind(this)
    this.getDistricts = this.getDistricts.bind(this)
    this.getWards = this.getWards.bind(this)
    this.fetchDistricts = this.fetchDistricts.bind(this)
    this.fetchWards = this.fetchWards.bind(this)
  }
    ...
    fetch(`https://vapi.vnappmob.com/api/province/district/${provincesID}`)
      ...
      .then(data => {
        ...
        if (districts !== undefined) {
          districts.map(value => document.getElementById('districts').innerHTML += `<option value='${value.district_id}'>${value.district_name}</option>`);
        }
      })
      .catch(error => {
        console.error('Lỗi khi gọi API:', error);
  ...
  fetchWards(districtsID) {
    fetch(`https://vapi.vnappmob.com/api/province/ward/${districtsID}`)
      ...
      .then(data => {
        ...
        document.getElementById('wards').innerHTML = `<option value=''>Select Ward</option>`;
        if (wards !== undefined) {
          wards.map(value => document.getElementById('wards').innerHTML += `<option value='${value.ward_id}'>${value.ward_name}</option>`);
        }
      })
      .catch(error => {
        console.error('Lỗi khi gọi API:', error);
  ...
  createNewDeliveryAddress = (accountId) => {
    ...
    let deliveryAddress = {
      ...
      specific_address: this.state.specificAddressRecipient + ', ' + this.state.ward + ', ' + this.state.district + ', ' + this.state.province + '.'
    };
    console.log('deliveryAddress => ' + JSON.stringify(deliveryAddress));
    DeliveryAddressServices.addDeliveryAddress(accountId, deliveryAddress).then((res) => {
    ...
    return (
      <>
                      ...
                      <div className="form-group d-flex align-items-center justify-content-between">
                        ...
                        <div>
                          ...
                          <button data-toggle="modal"
                            data-target={`#myModal`}
                            className="btn btn-primary rounded ">Add New Delivery Address</button>
                        </div>
                      </div>
                      {/* <div className="form-group">
                        ...
                        <div>
                          {this.state.deliveryAddress.length === 0 ? (
                            <div className="container-fluid">
                              <div className="row">
                                <div className="col-md-12 d-flex flex-column align-items-center">
                                  ...
                                  <h6 className="mb-2">
                                    I'm sorry! DrugMart couldn't find any delivery addresses in
                                    your cart.
                                  </h6>
                                  {/* <button
                                    className="btn btn-primary mb-4"
                            ...
                            this.state.deliveryAddress.map((delivery) => (
                              ...
                              <div style={{ border: '1px solid #ccc', backgroundColor: '#f2f6fe', }} className="row mt-3 mb-3 rounded p-3">
                                <div className="col-md-1">
                                  <div className="text-center  pt-1 pb-1" style={{ width: '100%', backgroundColor: '#d7ffcb', borderRadius: '50%' }}>
                                    <FontAwesomeIcon style={{ color: "" }} icon={faLocationDot} />
                                  </div>
                                </div>
                                <div className="col-md-10">
                                  <p><strong>{delivery.recipient_full_name}</strong> <span className="ml-2 mr-2"> | </span> <span>{delivery.recipient_phone_number}</span></p>
                                  <p>{delivery.specific_address}</p>
                                </div>
                                <div className="col-md-1">

                                  <button onClick={() => this.deleteDelivery_Address(accountId, delivery.address_id)} className="btn ">
                                    <FontAwesomeIcon icon={faTrash} />
                                  </button>
                                </div>
                              </div>
        ...
        <div class="container">
          ...
          >
            <div
              style={{ maxWidth: "700px", overflowY: "scroll", maxHeight: '86%' }}
                ...
                <div className=" container row mt-3">
                  ...
                  <div className=" modal-body col-md-12  ">
                    ...
                    <div class="form-group">
                      <input onChange={this.changeFullNameRecipient} className="form-control rounded" type="text" placeholder="Full Name" />

                    </div>
                    <div class="form-group">
                      <input onChange={this.changePhoneRecipient} className="form-control rounded" type="text" placeholder="Phone Number" />
                  ...
                  <div className=" modal-body col-md-12  ">
                    ...
                    <div class="form-group">
                      <select style={{ color: '757575', opacity: '1' }} className="form-control rounded" id='provinces' onchange={this.getProvinces}>
                        <option value=''>Select Province/City</option>
                      </select>
                    </div>
                    <div class="form-group">
                      <select style={{ color: '757575', opacity: '1' }} className="form-control rounded" id='districts' onchange={this.getDistricts}>
                        <option value=''>Select District</option>
                      </select>
                    </div>
                    <div class="form-group">
                      <select style={{ color: '757575', opacity: '1' }} className="form-control rounded" id='wards' onChange={this.getWards}>
